class NiftyMasterDetailsScaffoldGenerator < Rails::Generator::Base
  ...
  def manifest
    record do |m|
      ...
      unless options[:skip_controller]
        ...
        if list_partial?
          m.template "views/#{view_language}/_list.html.#{view_language}", "app/views/#{plural_name}/_list.html.#{view_language}"
        end
  ...
  #TODO #{action}_partial? will became partial? with a parameter to remove duplication
  def list_partial?
    actions? :index, :show
  end
  ...
  #TODO render_#{action} will became render with a parameter to remove duplication
  def render_list
    if list_partial?
      if options[:haml]
        "= render :partial => 'list'"
      ...
      read_template("views/#{view_language}/_list.html.#{view_language}")
    end
  end
